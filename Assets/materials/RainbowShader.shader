	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag (v2f i) : SV_Target {
				...
				// Select a new color every two row
				unsigned int c = (unsigned int)(i.uv.y * 256);
				c = (_Color + c) % 7 + 1;
				...
				float2 index = float2(shade * _PaletteNormWidth, c * _PaletteNormHeight);
